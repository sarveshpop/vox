...
const Add = () => {
  ...
  const filterUsers = (authenticatedUser, users) => {
    ...
    const filteredUsers = users.filter((user) =>
      isNotFriend(authenticatedUser, user)
    );
  ...
  const updateUsers = async (authenticatedUser, selectedUser) => {
    ...
    setIsLoading(true);
    const authenticatedUserPending = {
    ...
    authenticatedUser.waiting =
      authenticatedUser.waiting && authenticatedUser.waiting.length
        ? [...authenticatedUser.waiting, selectedUserPending]
        : [selectedUserPending];
    selectedUser.pending =
      selectedUser.pending && selectedUser.pending.length
        ? [...selectedUser.pending, authenticatedUserPending]
        : [authenticatedUserPending];
    await updateUser(authenticatedUser);
    await updateUser(selectedUser);
    setIsLoading(false);
    alert(`The request was sent to ${selectedUser.username} successfully`);
  };

  const onConfirmShown = async (selectedUser) => {
    const shoudRequestSent = window.confirm(
      `Would you like to add ${selectedUser.username} as a friend`
    );
    if (shoudRequestSent) {
