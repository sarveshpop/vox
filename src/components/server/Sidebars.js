...
import MenusServer from "../common/MenusServer";
...
const Sidebars = (props) => {
  ...
  const { cometChat, user, setSelectedChannel, setSelectedChannelType } =
    useContext(Context);
  ...
  const setActiveChannel = (selectedChannel) => {
    ...
    setTextChannels((previousChannels) =>
      previousChannels.map((channel) =>
        channel.guid === selectedChannel.guid
          ? { ...selectedChannel, isActive: true }
          : { ...channel, isActive: false }
      )
    );
    setVoiceChannels((previousChannels) =>
      previousChannels.map((channel) =>
        channel.guid === selectedChannel.guid
          ? { ...selectedChannel, isActive: true }
          : { ...channel, isActive: false }
  ...
  const joinFirebaseChannel = async ({
    latestSelectedChannel,
    user,
    convertedChannelType,
  }) => {
    if (!latestSelectedChannel || !user) {
      return;
    }
    latestSelectedChannel.members =
      latestSelectedChannel.members && latestSelectedChannel.members.length
        ? [...latestSelectedChannel.members, user.id]
        : [user.id];
    await await realTimeDb
      .ref(`${convertedChannelType}/${latestSelectedChannel.guid}`)
      .set(latestSelectedChannel);
  };
  ...
  const onChannelSelected = async (selectedChannel, channelType) => {
    ...
    const convertedChannelType =
      channelType === 1 ? "text-channels" : "voice-channels";
    const latestSelectedChannel = await getLatestSelectedChannel(
    ...
    if (
      latestSelectedChannel &&
      (!latestSelectedChannel.members ||
        !latestSelectedChannel.members.includes(user.id))
    ) {
      await joinFirebaseChannel({
        latestSelectedChannel,
        user,
        convertedChannelType,
      });
    ...
    <div className="server__sidebar">
      ...
      <div className="server__list">
        <Channels
          ...
          channelType={1}
          onItemClicked={onChannelSelected}
        />
        <Channels
